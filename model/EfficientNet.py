...
# https://github.com/YuanGongND/psla/blob/main/src/models/Models.py
...
if __name__ == '__main__':
    input_tdim = 64
    model = EffNetAttention(pretrain=False, b=2, head_num=1,label_dim=10)
    # input a batch of 10 spectrogram, each with 100 time frames and 128 frequency bins
    x = torch.rand([8,1, input_tdim, 128])
    x = model(x)
    print(x.shape)
    ...
    for p in model.parameters():
        if p.requires_grad:
            params += p.numel()
    print("Number of parameters: ", params/1e6)